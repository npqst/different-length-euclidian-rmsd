...
class DynamicTimeWarpDistance:
    ...
    def dtw_dist(
        ...
        if rmsd_equivalent is True:
            min_dist = self._dp_path_search(
                dist_mat, get_alignment=get_alignment, normalised=True
            )
            if get_alignment:
            ...
            min_dist = self._dp_path_search(
                dist_mat, get_alignment=get_alignment, normalised=normalised
            )
        return min_dist

    @staticmethod
    def _dp_path_search(dist_matrix, get_alignment=False, normalised=True):
        ...
        def select_min_cost_origin(m, n):
            if m > 0 and n > 0:
                if not get_alignment and not normalised:
                    ...
                    dtw_origins = np.asarray(
                        [dtw_mat[idx[0], idx[1]] for idx in indices]
                    )
            ...
            for j in range(0, dtw_mat.shape[1]):
                ...
                if normalised is True:
                    if idx_origin == (None, None):
                        ...
                        path_length_mat[i, j] = (
                            path_length_mat[idx_origin[0], idx_origin[1]] + 1
                        )
                if get_alignment:
    ...
    def _calc_dist_matrix(x, y):
        assert (
            x.shape[-1] == y.shape[-1]
        ), f"""Last dimension of coordinates
